...
bool EnvironmentNavXYThetaLatGeneric::InitializeEnv(int width, int height, const unsigned char* mapdata, double startx, double starty, double starttheta, double goalx, double goaly, double goaltheta, double goaltol_x, double goaltol_y, double goaltol_theta,
		const std::vector<sbpl_2Dpt_t>& perimeterptsV, double cellsize_m, double nominalvel_mpersecs, double timetoturn45degsinplace_secs, unsigned char obsthresh, const char* sMotPrimFile)
{
	nhPriv_.getParam("freespace_heuristic_costmap", freespace_heuristic_costmap_file_);
	if (! freespace_heuristic_costmap_file_.empty())
	{
		useFreespaceHeuristic_ = true;
		bool heuristic_map_valid = true;
		ROS_INFO("Loading freespace costmap from %s and enabling useFreespaceHeuristic.", freespace_heuristic_costmap_file_.c_str());
		...
		freespace_heuristic_costmap = boost::make_shared<freespace_mechanism_heuristic::HeuristicCostMap>(freespace_heuristic_costmap_file_, behavior);
	...
	return EnvironmentNAVXYTHETALAT::InitializeEnv(width, height, mapdata, startx, starty, starttheta, goalx, goaly, goaltheta, goaltol_x, goaltol_y, goaltol_theta, perimeterptsV, cellsize_m, nominalvel_mpersecs, timetoturn45degsinplace_secs, obsthresh, sMotPrimFile);
}
...
void EnvironmentNavXYThetaLatGeneric::updateForPlanRequest()
{
	...
	if (freespace_heuristic_costmap)
	{
		...
		{
			ROS_INFO("Switching freespace_costmap_out_of_map_behavior to %s", freespace_costmap_out_of_map_behavior.c_str());
			freespace_heuristic_costmap->setOutOfMapBehavior(freespace_mechanism_heuristic::HeuristicCostMap::getOutOfMapBehaviorFromString(freespace_costmap_out_of_map_behavior));
		}
	}
}
...
int EnvironmentNavXYThetaLatGeneric::GetGoalHeuristic(int stateID)
{
	...
	int hEuclid = (int) (NAVXYTHETALAT_COSTMULT_MTOMM * EuclideanDistance_m(x, y, EnvNAVXYTHETALATCfg.EndX_c, EnvNAVXYTHETALATCfg.EndY_c));
...
moveit_msgs::DisplayTrajectory EnvironmentNavXYThetaLatGeneric::pathToDisplayTrajectory(const std::vector<geometry_msgs::PoseStamped> & path) const
{
	...
	for (unsigned int i = 0; i < path.size(); ++i)
	{
		...
		if (i > 0)
		{
			...
			if (hypot(delta.getOrigin().x(), delta.getOrigin().y()) < 0.05 && fabs(tf::getYaw(delta.getRotation())) < 0.2)
				continue;
		}
